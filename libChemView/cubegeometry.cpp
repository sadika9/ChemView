...
CubeGeometry::~CubeGeometry()
{
    glDeleteBuffers(2, cubeVboIds);
}

void CubeGeometry::init(QString position, QString texCoord)
{
    m_position = position;
    m_texCoord = texCoord;
    ...
    glBindBuffer(GL_ARRAY_BUFFER, cubeVboIds[0]);
    glBufferData(GL_ARRAY_BUFFER, 24 * sizeof(VertexData), vertices, GL_STATIC_DRAW);

    // Transfer index data to VBO 1
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cubeVboIds[1]);
...
void CubeGeometry::drawGeometry(QOpenGLShaderProgram *program)
{
    ...
    int texcoordLocation = program->attributeLocation(m_texCoord);
...
void CubeGeometry::setShaderAttrs(QString position, QString texCoord)
{
    m_position = position;
    m_texCoord = texCoord;
}
